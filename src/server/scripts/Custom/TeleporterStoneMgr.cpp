...
{
    if (p_Reload)
    if (QueryResult l_Result = WorldDatabase.Query("SELECT ID, Text, CofnrimText, RefID, HardCodedID, RequiredVIPLevel, RequiredMG, MapID, X, Y, Z, O, LinkedIDS FROM z_teleporter_stone"))
    {
        do
        {
...
public:
    ...
    void OnGossipSelect(Player* p_Player, Item* p_Item, uint32 p_Sender, uint32 p_Action) override
    {
