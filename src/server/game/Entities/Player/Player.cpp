...
void Player::ApplyRatingMod(CombatRating combatRating, int32 value, bool apply)
{
    ...
    //Haste Cap Handled in UpdateRating
...
void Player::UpdateRating(CombatRating cr)
{
    ...
    float hasteMod = .504461369230769f;//0.25418596124031f - 12900
    float percentHasteMultiplier = 10.0f;

    /**********************************/
    int32 value = m_baseRatingValue[cr];
    // Apply bonus from SPELL_AURA_MOD_RATING_FROM_STAT
    // stat used stored in miscValueB for this aura
    AuraEffectList const& modRatingFromStat = GetAuraEffectsByType(SPELL_AURA_MOD_RATING_FROM_STAT);
    for (AuraEffect const* aurEff : modRatingFromStat)
        if (aurEff->GetMiscValue() & (1 << cr))
            value += int32(CalculatePct(GetStat(Stats(aurEff->GetMiscValueB())), aurEff->GetAmount()));

    if (value < 0)
        value = 0;
    SetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + cr, uint32(value));
    ...
    switch (cr)
    {
        ...
        case CR_HASTE_MELEE:
        {
            value += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKSPEED) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_HASTE) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_HASTE_2) * percentHasteMultiplier;
            //value += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKSPEED_2) * percentHasteMultiplier; Added in 5.0.5b
            value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_RANGED_HASTE) * percentHasteMultiplier;
            //value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_RANGED_HASTE_2) * percentHasteMultiplier; Added in 5.0.5b
            ...
            float atkSpd1 = speed1 * ((100.0f - ((value / 32.789989f) * hasteMod)) / 100.0f);
            if (atkSpd1 < 0)
                atkSpd1 = 0;
            float atkSpd2 = speed2 * ((100.0f - ((value / 32.789989f) * hasteMod)) / 100.0f);
            if (atkSpd2 < 0)
                atkSpd2 = 0;
            SetFloatValue(UNIT_FIELD_BASEATTACKTIME + BASE_ATTACK, atkSpd1);
            SetFloatValue(UNIT_FIELD_BASEATTACKTIME + OFF_ATTACK, atkSpd2);
        }
            break;
        case CR_HASTE_RANGED:
        {
            value += GetTotalAuraModifier(SPELL_AURA_HASTE_RANGED) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKSPEED) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_RANGED_HASTE) * percentHasteMultiplier;
           // value += GetTotalAuraModifier(SPELL_AURA_MOD_RANGED_HASTE_2) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_RANGED_HASTE) * percentHasteMultiplier;
           // value += GetTotalAuraModifier(SPELL_AURA_MOD_MELEE_RANGED_HASTE_2) * percentHasteMultiplier;

            Item * ranged = m_items[EQUIPMENT_SLOT_RANGED];
            float speed = ranged ? (float)ranged->GetTemplate()->Delay : 2000.0f;
            float rSpd = speed * ((100.0f - ((value / 32.789989f) * hasteMod)) / 100.0f);
            if (rSpd < 0)
                rSpd = 0;
            SetFloatValue(UNIT_FIELD_BASEATTACKTIME + RANGED_ATTACK, rSpd);
        }
            break;
        case CR_HASTE_SPELL:
        {
            value += GetTotalAuraModifier(SPELL_AURA_MELEE_SLOW) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_HASTE_SPELLS) * percentHasteMultiplier;
            value += GetTotalAuraModifier(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK) * percentHasteMultiplier;
            float sSpd = 1.0f * ((100.0f - ((value / 32.789989f) * hasteMod)) / 100.0f);//SpellSpeed handled in Unit::ModSpellCastTime
            SetFloatValue(UNIT_MOD_CAST_SPEED, sSpd);
        }
            break;
...
bool Player::CanRewardQuest(Quest const* quest, bool msg)
{
    ...
    if (GetCustomCurrency(MG) < quest->requiredMG)
    {
        ChatHandler(this->GetSession()).PSendSysMessage("|cFFFF0000You don't have enough MG to complete this quest! |R|cFF00E4FFYou need %u MG, but you only have %u MG, so you need %u more mg.", quest->requiredMG, GetCustomCurrency(MG), quest->requiredMG - GetCustomCurrency(MG));
        return false;
    }
    return true;
}
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    AddCustomCurrency(MG, quest->rewardMG, true);
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (GetCustomCurrency(MG) < pProto->reqMG)
    {
        ChatHandler(GetSession()).PSendSysMessage("Not enough MG! Need %u more!", uint32(pProto->reqMG - GetCustomCurrency(MG)));
        return false;
    }

	else
	{
		RemoveCustomCurrency(MG, pProto->reqMG, true);
	}
...
bool Player::RemoveCustomCurrency(CustomCurrency curr, int32 val, bool showToChat)
{
	...
	case MG:
		...
		break;
	case VP:
		LoginDatabase.PQuery("UPDATE auth.account SET vp = vp + %u WHERE id = %u", val, GetSession()->GetAccountId());
		if (showToChat)
			ChatHandler(this->GetSession()).PSendSysMessage("+%u VP!");
		break;
	case DP:
		LoginDatabase.PQuery("UPDATE auth.account SET dp = dp + %u WHERE id = %u", val, GetSession()->GetAccountId());
		if (showToChat)
			ChatHandler(this->GetSession()).PSendSysMessage("+%u DP!");
		break;
	}
...
bool Player::SetCustomCurrency(CustomCurrency curr, uint32 val, bool showToChat)
{
	...
	case MG:
		...
		break;
	case VP:
		LoginDatabase.PQuery("UPDATE auth.account SET vp = %u WHERE id = %u", val, GetSession()->GetAccountId());
		if (showToChat)
			ChatHandler(this->GetSession()).PSendSysMessage("%u VP!");
		break;
	case DP:
		LoginDatabase.PQuery("UPDATE auth.account SET dp = %u WHERE id = %u", val, GetSession()->GetAccountId());
		if (showToChat)
			ChatHandler(this->GetSession()).PSendSysMessage("%u DP!");
		break;
	}
...
uint32 Player::GetCustomCurrency(CustomCurrency curr)
{
	...
	case MG:
		...
		break;
	case VP:
		res = LoginDatabase.PQuery("SELECT vp from auth.account where id = %u", GetSession()->GetAccountId());
		if (res)
		{
		...
		break;
	case DP:
		res = LoginDatabase.PQuery("SELECT dp from auth.account where id = %u", GetSession()->GetAccountId());
		if (res)
		{
			Field* fields = res->Fetch();
			return fields[0].GetUInt32();
